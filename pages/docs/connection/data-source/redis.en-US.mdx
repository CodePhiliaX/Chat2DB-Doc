...
Use Chat2DB to quickly connect to Redis server

## Step 1: Create a new connection

!['redis2'](/image/connection/redis2.png)

Click the database button on the left, select New Connection, and select Redis
...
- `Host`: Redis server address, which can be IP or domain name
...
- `URL`: Redis connection URL, optional. If not filled in, it will be automatically generated based on the above information.
- `Driver`: Redis driver, optional. If not filled in, it will be automatically identified based on the URL. You can also select it manually. After manual selection, the driver configuration items will be displayed.
...
If you do not select a driver, you need to download the Redis driver. Click the download button `Download Driver` to download the latest Redis driver. You can also manually upload your own driver `Upload driver`
