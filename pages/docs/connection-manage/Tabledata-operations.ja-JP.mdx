# テーブルデータに対する関連操作

テーブル上の一連の操作については、個別に紹介します。

!['total'](/image/connection-manage/total.png)

- `Open Table`：テーブル構造を開いてデータを表示します
- `Query console`：このデータベースに基づいて新しいコンソールを作成します
- `Pin`：固定されたテーブル
- `Copy name`: テーブル名をコピーする
- `View DDL`：テーブルの DDL ステートメントを表示する
- `Copy table`: テーブルをコピーします。テーブル構造をコピーするか、テーブル データをコピーするかを選択できます。
- `Truncate Table`: テーブルをクリアします。
- `Edit Table`: テーブルを編集します (テーブル名、コメント、列名、主キーなど)。
- `Export as SQL file`: SQL ファイルとしてエクスポートします。テーブル構造とテーブルデータのエクスポートを選択できます
- `Import Data`: CSV、XLSX、XLS、SQL 形式でファイルをインポートします。
- `Export Data`: データ ファイルを CSV、XLSX、XLS、SQL 形式にエクスポートします。
- `Delete Table`：テーブルの削除
- `Generate Test Data`：開発者が使用するテストデータを生成する

## データを表示する

データベース エクスプローラー (ビュー | ツール ウィンドウ | データベース エクスプローラー) でテーブルを**ダブルクリック**すると、データ エディターでテーブル ビューが開きます。
または、マウスを右クリックして表示したいテーブルを選択し、`Open Table`をクリックして表示することもできます。

!['visualization1'](/image/connection-manage/visualization1.png)

## データを編集する

テーブル内で直接変更を加えることができます。データを変更した後、送信すると変更が成功します。

!['update'](/image/connection-manage/directUpdate.png)

フィールド名が長すぎてテーブルから直接変更または表示できない場合は、表示または変更するデータを選択し、右クリックして`View/Edit Data`をクリックしてページを表示および編集できます。

!['view and edit'](/image/connection-manage/VE_data.png)

さらに、特定の変更操作の DDL ステートメントを表示することもできます。

!['update2'](/image/connection-manage/update2.png)


## データを更新する

エディターに表示されるデータをデータベースの内容と同期したい場合は、テーブル ビューのデータを再ロードする必要があります。

- ツールバーの`refresh`ボタンをクリックします。

!['refresh1'](/image/connection-manage/refresh1.png)

- テーブルを右クリックし、コンテキスト メニューから ページの再読み込み を選択します。

!['refresh2'](/image/connection-manage/refresh2.png)

## 行と行の操作

!['operations'](/image/connection-manage/operations_data2.png)
データ行を選択するときにマウスを右クリックすると、次の操作が実行されます。

- `Insert SQL`: データ行全体を Insert SQL ステートメントにコピーします。

!['Insert SQL'](/image/connection-manage/Insert_SQL.png)

- `Update SQL`: データ行全体を Update SQL ステートメントにコピーします。

!['Update SQL'](/image/connection-manage/Update_SQL.png)

- `TAB delimited (data)`: データ行全体をコピーします。

!['TAB delimited'](/image/connection-manage/TAB_data.png)

- `TAB delimited (field name)`: データ行全体に対応するフィールド名をコピーします。

!['TAB delimited'](/image/connection-manage/TAB_field.png)

- `Tab-separated (field names and data)`: データの行全体とそれに対応するフィールド名をコピーします。

!['TAB delimited'](/image/connection-manage/TAB_all.png)

- `Clone row`: データ行全体のクローンを作成します。

- `Delete row`: データの行全体を削除します。

**チップ**：
- 複数行のデータをコピーする場合は、「Shift + マウスの左ボタン」を使用して複数の行を選択して操作してください。
- テーブル内のデータの行全体をコピーする場合は、マウスの左上隅にある空白領域を左クリックして、すべてを選択します (以下を参照)。

!['select all'](/image/connection-manage/selectAll.png)

## テーブルデータの操作

!['operations2'](/image/connection-manage/operations_data.png)

- `View/Edit Data`: データの表示/編集
- `Copy`: 選択した単一データをコピーします
- `Copy the row as`: 上と同じ...
- `Clone row`: 行データのクローンを作成します。
- `Set NULL`: テーブルデータを **null** に設定します
- `Set DEFAULT`: テーブル データを **デフォルト値** に設定します
- `Delete row`: 行を削除します。

