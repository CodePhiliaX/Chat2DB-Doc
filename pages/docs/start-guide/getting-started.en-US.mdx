...
You can try Chat2DB for free for up to 30 days, with 20 free AI trials and can connect to multiple databases at the same time.

After the PRO trial expires, you need to purchase a membership to continue using Chat2DB.

You can get the verification code directly through your email and log in directly, or you can use a third party to log in. Currently supports multiple login methods such as Github and Google.

[Login address](https://app.chat2db.ai/login)

Chat2DB currently supports 5 login methods:
1. Google login
2. GitHub login
3. WeChat login
4. Log in with email verification code
...
## Step 6: Use Chat2DB data analysis

!['resultDashboard'](/image/connection-manage/resultDashboard.png)

Click `Dashboard` on the left, then click the "+" sign to create a new report, fill in the report information, then select the report type you need, and click Save.
